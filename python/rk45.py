...
def solve(f, t0, tfinal, y0, tol=1e-7):
  ...
  """
    ...
    while t < tfinal:
        ...
        k[3] = F(t + a[3] * h, y + h *
                 (k[0] * b[3][0] + k[1] * b[3][1] + k[2] * b[3][2]))
        k[4] = F(t + a[4] * h, y + h * (k[0] * b[4][0] + k[1] * b[4][1] + k[2]
                                        * b[4][2] + k[3] * b[4][3]))
        k[5] = F(t + a[5] * h, y + h *
                 (k[0] * b[5][0] + k[1] * b[5][1] + k[2] * b[5][2] + k[3] *
                  b[5][3] + k[4] * b[5][4]))

        # Estimate current error and current maximum error.
        E = norm(h * (k[0] * dc[0] + k[1] * dc[1] + k[2] * dc[2] + k[3] * dc[3]
                      + k[4] * dc[4] + k[5] * dc[5]))
        ...
        if E < Emax:
            t += h
            y += h * (k[0] * c[0] + k[1] * c[1] + k[2] * c[2] + k[3] * c[3] +
                      k[4] * c[4] + k[5] * c[5])
            out += [(t, list(y))]
  ...
  """
    ...
    def yp(t, y):
        ...
        return (u1p,
                ...
                u2p, u2 - 2 * u1p - muhat * u2 / D1 - mu * u2 / D2)
