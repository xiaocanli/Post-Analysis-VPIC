...
unique name for your PIC run. The file will be saved in "../data/pic_info/"
"""
import collections
import errno
import math
...
from serialize_json import data_to_json
...
def get_pic_info(base_directory, run_name):
    ...
    fields_interval, particle_interval, trace_interval = get_output_intervals(
        dtwpe, dtwce, dtwpi, dtwci, base_directory, deck_file)
    ...
    pic_run_info = pic_run(run_dir=base_directory, run_name=run_name)
...
def get_main_source_filename(base_directory):
    ...
    try:
        ...
        f.close()
        current_line = 0
        cond1 = '.cxx' not in content[current_line]
        cond2 = '.cc' not in content[current_line]
...
def get_output_intervals(dtwpe, dtwce, dtwpi, dtwci, base_directory,
    ...
    try:
        ...
        f.close()
        current_line = 0
        cond1 = 'int interval = ' not in content[current_line]
        ...
        if '(' not in content[current_line]:
            ...
            cond1 = word not in content[current_line]
            cond2 = '//' in content[current_line]  # commented out
            while cond1 or cond2:
                current_line -= 1
                cond1 = word not in content[current_line]
        ...
        while 'int eparticle_interval' not in content[current_line]:
            current_line += 1
        single_line = content[current_line]
        line_splits = single_line.split("=")
        if '*' in line_splits[1]:
...
def read_pic_info(base_directory):
    ...
    te, current_line = get_variable_value('Te',
                                          current_line,
                                          content,
                                          match_name=True)
    ti, current_line = get_variable_value('Ti',
                                          current_line,
                                          content,
                                          match_name=True)
    wpe_wce, current_line = get_variable_value('wpe/wce', current_line,
                                               content)
    ...
    courant, current_line = get_variable_value('courant', current_line,
                                               content)
    nproc, current_line = get_variable_value('nproc', current_line, content,
                                             int)
    ...
    dx_rhoi, current_line = get_variable_value('dx/rhoi', current_line,
                                               content)
    dx_rhoe, current_line = get_variable_value('dx/rhoe', current_line,
                                               content)
    dx_debye, current_line = get_variable_value('dx/debye', current_line,
                                                content)
    ...
    if any('vthi/c' in s for s in content):
        ...
        vthe = 1.0
        vthi = 1.0
    restart_interval, current_line = get_variable_value(
        'restart_interval', current_line, content, int)
    fields_interval_info, current_line = get_variable_value(
        'fields_interval', current_line, content, int)
    ...
    Hhydro_interval, current_line = get_variable_value('Hhydro_interval',
                                                       current_line, content,
                                                       int)
    eparticle_interval, current_line = get_variable_value(
        'eparticle_interval', current_line, content, int)
    Hparticle_interval, current_line = get_variable_value(
        'Hparticle_interval', current_line, content, int)
    quota_check_interval, current_line = get_variable_value(
        'quota_check_interval', current_line, content, int)
    particle_tracing, current_line = get_variable_value(
        'particle_tracing', current_line, content, int)
    tracer_interval, current_line = get_variable_value('tracer_interval',
                                                       current_line, content,
                                                       int)
    tracer_pass1_interval, current_line = get_variable_value(
        'tracer_pass1_interval', current_line, content, int)
    tracer_pass2_interval, current_line = get_variable_value(
        'tracer_pass2_interval', current_line, content, int)
    ntracer, current_line = get_variable_value('Ntracer', current_line,
                                               content, int)
    ...
    hydro_at_tracer, current_line = get_variable_value('hydro_at_tracer',
                                                       current_line, content,
                                                       int)
    dump_traj_directly, current_line = get_variable_value(
        'dump_traj_directly', current_line, content, int)
    num_tracer_fields_add, current_line = get_variable_value(
        'num_tracer_fields_add', current_line, content, int)
    emax_band, current_line = get_variable_value('emax_band', current_line,
                                                 content)
    emin_band, current_line = get_variable_value('emin_band', current_line,
                                                 content)
    nbands, current_line = get_variable_value('nbands', current_line, content,
                                              int)
    emax_spect, current_line = get_variable_value('emax_spect', current_line,
                                                  content)
    emin_spect, current_line = get_variable_value('emin_spect', current_line,
                                                  content)
    nbins_spect, current_line = get_variable_value('nbins', current_line,
                                                   content, int)
    nx_zone, current_line = get_variable_value('nx_zone', current_line,
                                               content, int)
    ny_zone, current_line = get_variable_value('ny_zone', current_line,
                                               content, int)
    nz_zone, current_line = get_variable_value('nz_zone', current_line,
                                               content, int)
    stride_particle_dump, current_line = get_variable_value(
        'stride_particle_dump', current_line, content, int)

    pic_init_info = collections.namedtuple('pic_init_info', [
        'sigmae_c', 'ti_te', 'Ti', 'Te', 'wpe_wce', 'mime', 'lx_di', 'ly_di',
        'lz_di', 'nx', 'ny', 'nz', 'courant', 'nproc', 'nppc', 'b0', 'ne',
        'dtwpe', 'dtwce', 'dtwci', 'dx_di', 'dy_di', 'dz_di', 'x_di', 'y_di',
        'z_di', 'dx_rhoi', 'dx_rhoe', 'dx_debye', 'n0', 'restart_interval',
        'fields_interval_info', 'ehydro_interval', 'Hhydro_interval',
        'eparticle_interval', 'Hparticle_interval', 'energy_interval',
        'quota_check_interval', 'particle_tracing', 'tracer_interval',
        'tracer_pass1_interval', 'tracer_pass2_interval', 'ntracer',
        'emf_at_tracer', 'hydro_at_tracer', 'dump_traj_directly',
        'num_tracer_fields_add', 'emax_band', 'emin_band', 'nbands',
        'emax_spect', 'emin_spect', 'nbins_spect', 'nx_zone', 'ny_zone',
        'nz_zone', 'stride_particle_dump', 'vthi', 'vthe'
    ])
    pic_info = pic_init_info(sigmae_c=sigmae_c,
                             ti_te=ti_te,
                             Te=te,
                             Ti=ti,
                             wpe_wce=wpe_wce,
                             ...
                             emin_band=emin_band,
                             nbands=nbands,
                             emax_spect=emax_spect,
                             emin_spect=emin_spect,
...
def get_variable_value(variable_name,
    ...
    cond1 = variable_name not in single_line
...
def get_pic_topology(base_directory, deck_file):
    ...
    try:
        ...
        f.close()
        current_line = 0
        cond1 = 'double topology_x =' not in content[current_line]
