...
def plot_four_panels(datah, datav, cond_exclude_cs, fname, ixyz, twpe10,
                     var="wpara_wperp", exclude_cs=False, show_plot=True,
                     wpara_wperp=False):
    ...
    for row, col in itertools.product(range(2), range(2)):
        ...
        labels = set_labels(ixyz, xsign, ysign, var, exclude_cs)
...
def create_fig_dir(pic_run, var='wpara_wperp', exclude_cs=False):
    ...
    Args:
        ...
        exclude_cs(optional): whether to exclude current sheet particles
    """
    top_dir = '../img/trans_relativistic/'
    var_name = var + '_four'
...
def set_fig_name(fig_dir, ixyz, istep, species='e',
                 var='wpara_wperp', exclude_cs=False):
    """Save figure name
...
def set_labels(ixyz, xsign, ysign, var='wpara_wperp', exclude_cs=False):
...
def compare_energization_four(plot_config, show_plot=True):
    ...
    exclude_cs = plot_config["exclude_cs"]
    var = plot_config["var_four"]
    fig_dir = create_fig_dir(pic_run, var, exclude_cs)
    fig_dir1 = create_fig_dir(pic_run, 'wpara_wperp', exclude_cs)
    fig_dir2 = create_fig_dir(pic_run, 'wpara_dgamma', exclude_cs)
    fig_dir3 = create_fig_dir(pic_run, 'wperp_dgamma', exclude_cs)

    # Loop over time frames
    nsteps = plot_config["nsteps"]
    for tframe, step in itertools.product(range(nframes), range(nsteps)):
        print("Time frame %d of %d" % (tframe, nframes))
        tindex0 = tframe * pic_info.tracer_interval * plot_config["nsteps"]
        if step == 0:
            fname = (tracer_dir + 'T.' + str(tindex0) + '/' +
        ...
        istep = tindex // pic_info.tracer_interval
        if istep % plot_interval == 0 and istep != 0:
            for ixyz in range(3, 4):
                fig_name = set_fig_name(fig_dir, ixyz, istep,
                                        species, var, exclude_cs)
                fig_name1 = set_fig_name(fig_dir1, ixyz, istep,
                                         species, var, exclude_cs)
                fig_name2 = set_fig_name(fig_dir2, ixyz, istep,
                                         species, var, exclude_cs)
                fig_name3 = set_fig_name(fig_dir3, ixyz, istep,
                                         species, var, exclude_cs)
                twpe10 = tindex * pic_info.dtwpe / 0.1
                if var == 'wpara_wperp':
                    plot_four_panels(dene_para[ixyz], dene_perp[ixyz],
                                     cond_exclude_cs, fig_name, ixyz,
                                     twpe10, var, exclude_cs, show_plot=False)
                elif var == 'wpara_dgamma':
                    plot_four_panels(dene_para[ixyz], dgamma,
                                     cond_exclude_cs, fig_name, ixyz,
                                     twpe10, var, exclude_cs, show_plot=False)
                elif var == 'wperp_dgamma':
                    plot_four_panels(dene_perp[ixyz], dgamma,
                                     cond_exclude_cs, fig_name, ixyz,
                                     twpe10, var, exclude_cs, show_plot=False)
                elif var == 'wtot_dgamma':
                    plot_four_panels(dene_para[ixyz]+dene_perp[ixyz], dgamma,
                                     cond_exclude_cs, fig_name, ixyz,
                                     twpe10, var, exclude_cs, show_plot=False)
                # plot_four_panels(dene_para[ixyz], dene_perp[ixyz],
                #                  cond_exclude_cs, fig_name1, ixyz,
                #                  twpe10, var, exclude_cs, show_plot=False)
                # plot_four_panels(dene_para[ixyz], dgamma,
                #                  cond_exclude_cs, fig_name2, ixyz,
                #                  twpe10, var, exclude_cs, show_plot=False)
                # plot_four_panels(dene_perp[ixyz], dgamma,
                #                  cond_exclude_cs, fig_name3, ixyz,
                #                  twpe10, var, exclude_cs, show_plot=False)
        if istep in [200, 3200, 7000, 14400]:
            out_dir = '../data/trans_relativistic/energization/'
            out_dir += pic_run + '/'
            if exclude_cs:
                out_dir += 'nocs/'
            else:
                out_dir += 'all/'
            mkdir_p(out_dir)
            fname = out_dir + 'energization_' + species + '_' + str(istep) + '.h5'
            with h5py.File(fname, 'w') as fh_out:
                fh_out.create_dataset('wpara_x', (nptl, ), data=dene_para[0])
                fh_out.create_dataset('wpara_y', (nptl, ), data=dene_para[1])
                fh_out.create_dataset('wpara_z', (nptl, ), data=dene_para[2])
                fh_out.create_dataset('wpara', (nptl, ), data=dene_para[3])
                fh_out.create_dataset('wperp_x', (nptl, ), data=dene_perp[0])
                fh_out.create_dataset('wperp_y', (nptl, ), data=dene_perp[1])
                fh_out.create_dataset('wperp_z', (nptl, ), data=dene_perp[2])
                fh_out.create_dataset('wperp', (nptl, ), data=dene_perp[3])
                fh_out.create_dataset('dgamma', (nptl, ), data=dgamma)
                fh_out.create_dataset('tag', (nptl, ), data=ptl["q"])
...
def plot_wpara_wperp(plot_config, show_plot=True):
    ...
    if plot_config["wpp_hdf5"]:
        root_dir = pic_run_dir + 'wpara_wperp_2nd_pass/'
    ...
    for fdir in [fdir_all, fdir_nocs]:
        ...
        label1 = r'$\sum W_\parallel/\sigma_e$' + plabel
        label2 = r'$\sum W_\perp/\sigma_e$' + plabel
        ax3.plot(ttracer, wpara_tot/sigma_e, linewidth=2,
                 linestyle=lstyle, label=label1)
        ax3.plot(ttracer, wperp_tot/sigma_e, linewidth=2,
                 linestyle=lstyle, label=label2)
...
def egain_after_injection(plot_config, show_plot=True):
    ...
    for fdir in [fdir_all, fdir_nocs]:
        ...
        ax1.plot(ttracer, wpara_post_avg, linewidth=2,
                 linestyle=lstyle, label=label1, color=COLORS[0])
        ax1.plot(ttracer, wperp_post_avg, linewidth=2,
                 linestyle=lstyle, label=label2, color='k')
        ax1.legend(loc=7, prop={'size': 12}, ncol=1,
                   shadow=False, fancybox=False, frameon=False)
        ax1.set_xlim([ttracer.min(), ttracer.max()])
        ax1.tick_params(bottom=True, top=True, left=True, right=True)
        ax1.tick_params(axis='x', which='minor', direction='in')
...
def compare_energization_four_h5(plot_config, show_plot=True):
    ...
    fig_dir2 = create_fig_dir(pic_run, 'wpara_dgamma', exclude_cs)
    fig_dir3 = create_fig_dir(pic_run, 'wperp_dgamma', exclude_cs)
    fig_dir4 = create_fig_dir(pic_run, 'wtot_dgamma', exclude_cs)
    ...
    ptl["wperp"] = np.zeros(nptl)
    ptl["dgamma"] = np.zeros(nptl, np.float32)
    cond_exclude_cs = np.ones(nptl)
    # for file_name in file_list[::-1]:
    for file_name in file_list[300:600]:
        print("File name: %s" % file_name)
        fsplit = file_name.split(".")
        tindex = int(fsplit[0].split("_")[-1])
        fname = fdir + file_name
        with h5py.File(fname, 'r') as fh:
            ...
            fig_name1 = set_fig_name(fig_dir1, 3, tindex, species,
                                     "wpara_wperp", exclude_cs)
            fig_name2 = set_fig_name(fig_dir2, 3, tindex, species,
                                     "wpara_dgamma", exclude_cs)
            fig_name3 = set_fig_name(fig_dir3, 3, tindex, species,
                                     "wperp_dgamma", exclude_cs)
            fig_name4 = set_fig_name(fig_dir4, 3, tindex, species,
                                     "wtot_dgamma", exclude_cs)
            twpe10 = tindex * pic_info.dtwpe / 0.1
            plot_four_panels(ptl["wpara"], ptl["wperp"], cond_exclude_cs, fig_name1,
                             3, twpe10, "wpara_wperp", exclude_cs, show_plot=False,
                             wpara_wperp=True)
            plot_four_panels(ptl["wpara"], ptl["dgamma"], cond_exclude_cs, fig_name2,
                             3, twpe10, "wpara_dgamma", exclude_cs, show_plot=False)
            plot_four_panels(ptl["wperp"], ptl["dgamma"], cond_exclude_cs, fig_name3,
                             3, twpe10, "wperp_dgamma", exclude_cs, show_plot=False)
            # plot_four_panels(ptl["wpara"] + ptl["wperp"], ptl["dgamma"],
            #                  cond_exclude_cs, fig_name4, 3, twpe10, "wtot_dgamma",
            #                  exclude_cs, show_plot=False)
...
def plot_absj(plot_config, show_plot=True):
    ...
    img_dir = '../img/trans_relativistic/absj/' + pic_run + '/'
    mkdir_p(img_dir)
...
def plot_absj_tri(plot_config, show_plot=True):
    ...
    cbar.ax.tick_params(labelsize=12)
    img_dir = '../img/trans_relativistic/absj_tri/' + pic_run + '/'
    mkdir_p(img_dir)
...
def get_cmd_args():
    ...
    """
    default_pic_run = 'sigma03_bg01_2720de_Lde14_triggered'
    default_pic_run_dir = ('/net/scratch3/xiaocanli/reconnection/trans-relativistic/' +
                           default_pic_run + '/')
...
def analysis_single_frames(plot_config, args):
    ...
    if args.plot_spect:
        ...
        compare_energization_four(plot_config)
