...
def read_3d_fields(pic_info, fname):
    ...
    fp = np.memmap(fname, dtype='float32', mode='r', 
            offset=0, shape=(nx,ny,nz), order='C')
    return fp
...
def plot_3d_fields(pic_info, fname, iy, tidex):
    ...
    vmin, vmax = -0.1, 0.1
    p1 = ax.imshow(data, cmap=plt.cm.seismic,
    ...
    ax.text(0.05, 0.9, fname, color='k', fontsize=32,
...
def plot_j_jdote(pic_info, root_dir, iy, tindex, tint):
    ...
    p1 = ax.imshow(jdote, cmap=plt.cm.seismic,
            extent=[xmin, xmax, ymin, ymax],
            aspect='auto', origin='lower',
            vmin=-0.01, vmax=0.01,
            interpolation='bicubic')
    ...
    ax.tick_params(labelsize=16)
    fname = r'$t\omega_{pe} = ' + str(tindex) + '$'
    cbar.ax.set_ylabel(r'$\boldsymbol{j}\cdot\boldsymbol{E}$',
    ...
    p1 = ax1.imshow(absJ0, cmap=plt.cm.jet,
            extent=[xmin, xmax, ymin, ymax],
            aspect='auto', origin='lower',
            vmin=0, vmax=0.3,
            interpolation='bicubic')
    ...
    ax1.text(-0.45, -0.11, fname, color='k', fontsize=32,
    ...
    p1 = ax2.imshow(eperp, cmap=plt.cm.seismic,
            extent=[xmin, xmax, ymin, ymax],
            aspect='auto', origin='lower',
            # vmin=-0.05, vmax=0.05,
            vmin=0, vmax=0.2,
...
def calc_parallel_potential(pic_info, root_dir, iy, tidex):
    ...
    edotb = (ex*bx + ey*by + ez*bz) / np.sqrt(bx*bx + by*by + bz*bz)
    ...
    print dmin, dmax
    p1 = ax.imshow(edotb_cum, cmap=plt.cm.seismic,
            extent=[xmin, xmax, ymin, ymax],
            aspect='auto', origin='lower',
            vmin=-1.0, vmax=1.0,
            interpolation='bicubic')
    ...
    ax.tick_params(labelsize=16)
    fname = r'$t\omega_{pe} = ' + str(tindex) + '$'
    cbar.ax.set_ylabel(r'$\int_0^{z_\text{max}}E_\parallel dz$',
            fontdict=font, fontsize=24)
    fname = 'max: ' + dmax
    ax.text(0.8, -0.08, fname, color='k', fontsize=20,
            horizontalalignment='left', verticalalignment='center',
            transform = ax.transAxes)
    fname = 'min: ' + dmin
    ax.text(0.8, -0.14, fname, color='k', fontsize=20,
            horizontalalignment='left', verticalalignment='center',
            transform = ax.transAxes)
...
def plot_epara(pic_info, root_dir, iy, tidex):
    ...
    p1 = ax.imshow(edotb_para, cmap=plt.cm.seismic,
            extent=[xmin, xmax, ymin, ymax],
            aspect='auto', origin='lower',
            vmin=-0.1, vmax=0.1,
            interpolation='bicubic')
