...
class Viewer2d(object):
    def __init__(self, file, nptl, x, y, fdata, init_ft, var_field, var_name,
        ...
        self.gama = np.sqrt(self.ptl.ux**2 + self.ptl.uy**2 +
                self.ptl.uz**2 + 1.0)
        ...
        self.pxe, = self.xe_axis.plot(self.pxb, self.gama - 1.0,
                linewidth=2, color='r')
        ...
        self.pye, = self.ye_axis.plot(self.pyb, self.gama - 1.0,
                linewidth=2, color='g')
        ...
        self.pze, = self.ze_axis.plot(self.pzb, self.gama - 1.0,
                linewidth=2, color='b')
...
class Viewer3d(object):
    def __init__(self, file, nptl, init_ft, var_field, var_name, species):
        ...
        for i in xrange(self.nt-1):
            self.pxz_axis.plot(self.pxb[i:i+2], self.pyb[i:i+2],
                    self.pzb[i:i+2], linewidth=2,
        ...
        # Energy plot
        self.ene_axis = self.fig.add_axes([0.1, 0.46, 0.35, 0.2])
        self.pene, = self.ene_axis.plot(self.tnew, self.gama - 1.0,
                linewidth=2, color='k')
        self.ene_axis.set_ylabel(r'$\gamma - 1$', fontdict=font, fontsize=20)
        self.ene_axis.set_xlabel(r'Time', fontdict=font, fontsize=20)
        self.ene_axis.tick_params(labelsize=16)
...
class traj3d(object):
    def __init__(self, file, nptl, iptl, init_ft, var_field, var_name, species):
        """
        Shows a given array in a 2d-viewer.
        Input: z, an 2d array.
        x,y coordinters are optional.
    ...
    def pxyz_plot(self):
        ...
        xs0, ys0 = 0.05, 0.07
        w1, h1 = 0.4, 0.9
        self.pxz_axis = self.fig_pxyz.add_axes([xs0, ys0, w1, h1],
                projection='3d')
        ...
        yLabel = self.pxz_axis.set_ylabel('$y$', fontsize=24)
        zLabel = self.pxz_axis.set_zlabel('$z$', fontsize=24)
        max_range = np.array([self.pxb.max()-self.pxb.min(),
                              self.pyb.max()-self.pyb.min(),
                              self.pzb.max()-self.pzb.min()]).max() / 2.0
...
class ParticleInfo(object):
    ...
    def get_particle_info(self, group):
        ...
        self.ptl = particles(x=self.x, y=self.y, z=self.z,
                ux=self.ux, uy=self.uy, uz=self.uz, q=self.q, t=self.t)
        return self.ptl
