...
class Viewer2d(object):
    def __init__(self, file, nptl, x, y, fdata):
        """
        ...
        group = file[self.particle_tags[1]]
        dset_ux = group['Ux']
        ...
        # xz plot
        self.pxz, = self.pxz_axis.plot(self.ptl.x, self.ptl.z, linewidth=2,
                color='k')
        ...
        self.pxe, = self.xe_axis.plot(self.ptl.x, self.gama - 1.0,
                linewidth=2, color='r')
        ...
        self.pye, = self.ye_axis.plot(self.ptl.y, self.gama - 1.0,
                linewidth=2, color='g')
        ...
        self.pze, = self.ze_axis.plot(self.ptl.z, self.gama - 1.0,
                linewidth=2, color='b')
        ...
        self.slider = DiscreteSlider(self.sliderax, 'Particle', 1, self.nptl,\
                allowed_vals=particle_list, valinit=particle_list[0])
        ...
        tframes = np.arange(1, pic_info.ntf)
        self.field_slider = DiscreteSlider(self.field_sliderax, 'Field', 1,
                pic_info.ntf, allowed_vals=tframes, valinit=tframes[19])
        self.field_slider.on_changed(self.update_field)

        self._widgets=[self.slider, self.field_slider]

    def update_particle(self, val):
        iptl = self.slider.val
        group = file[self.particle_tags[iptl]]
        self.ptl = self.particle_info.get_particle_info(group)
        self.gama = np.sqrt(self.ptl.ux**2 + self.ptl.uy**2 + self.ptl.uz**2 + 1.0)
        self.pxz.set_xdata(self.ptl.x)
        self.pxz.set_ydata(self.ptl.z)
        self.pene.set_ydata(self.gama - 1.0)
        self.pxe.set_xdata(self.ptl.x)
        self.pye.set_xdata(self.ptl.y)
        self.pze.set_xdata(self.ptl.z)
    ...
    def update_field(self, val):
        ct = self.field_slider.val
        print ct
        pic_info = pic_information.get_pic_info('../../')
        kwargs = {"current_time":ct, "xl":0, "xr":330, "zb":-40, "zt":40}
        fname = "../../data/jy.gda"
        x, z, self.fdata = read_2d_fields(pic_info, fname, **kwargs) 
...
if __name__ == "__main__":
    pic_info = pic_information.get_pic_info('../../')
    fname = '/scratch3/xiaocanli/open3d-h5tracer/pic_analysis/vpic-sorter/data/test.h5p'
    file = h5py.File(fname, 'r')
    ngroups = len(file)
    kwargs = {"current_time":20, "xl":0, "xr":330, "zb":-40, "zt":40}
    fname = "../../data/jy.gda"
    x, z, data = read_2d_fields(pic_info, fname, **kwargs) 
    fig_v = Viewer2d(file, ngroups, x, z, data)
    plt.show()
    file.close()
